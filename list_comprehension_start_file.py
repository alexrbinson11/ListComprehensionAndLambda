...
#adding an IF condition
    ...
    #get the sqaure of every even number from 0-10
...
wordlength = [int(len(x)) for x in words if x!= 'the']
print(wordlength)
...
weight = [x.upper() for x,y in dict.items() if y < 5000]
print(weight)
...
contain = [i for i in range(1,1001) if i % 10 == 4]
print(contain)

## count how many times the word 'the' appears in the text file - 'sometext.txt'
infile = open('sometext.txt', 'r')
total = len([x for x in infile.read().split() if x.lower() == "the"])
print(total)
